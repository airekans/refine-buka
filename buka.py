...
try:
	...
	print(
            "dwebp is not available, so only normal image files are supported.\n" + repr(ex))
	supportwebp = False
...
if os.path.isdir(target):
	if os.path.splitext(fn_buka)[1] == ".buka":
		...
		if os.path.isfile(os.path.join(target, "chaporder.dat")):
			...
			newtarget = os.path.join(os.path.dirname(target), dat[
			                         'name'] + '-' + renamef(chap, os.path.basename(os.path.splitext(fn_buka)[0])))
		...
		for name in files:
			...
			if os.path.splitext(fpath)[1] == ".buka":
				...
				if os.path.isfile(chaporder):
					...
					os.remove(chaporder)
					chap = build_dict(dat['links'], 'cid')
					shutil.move(os.path.splitext(fpath)[0], os.path.join(os.path.dirname(
                                            fpath), renamef(chap, os.path.basename(os.path.splitext(fpath)[0]))))
				os.remove(fpath)
	for root, subFolders, files in os.walk(target):
		for name in files:
		...
		if os.path.splitext(fpath)[1] in (".view", ".bup"):
			...
			if os.path.splitext(bupname)[1] == ".bup":
				if supportwebp:
					...
					os.remove(fpath)
					p = Popen([dwebp, basepath + ".webp", "-o", os.path.splitext(basepath)
                                            [0] + ".png"], cwd=os.getcwd())  # .wait()  faster
		...
		if os.path.isfile(os.path.join(fpath, "chaporder.dat")):
			...
			for item in os.listdir(fpath):
				if os.path.isdir(os.path.join(fpath, item)):
					shutil.move(
                                            os.path.join(fpath, item), os.path.join(fpath, renamef(chap, item)))
